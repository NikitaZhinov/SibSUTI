...
int main() {
    ChessGame chess(sf::Color(0xf3bc7aff), sf::Color(0xae722bff)); // Светло-жёлтый и темно коричневый

    sf::RenderWindow window(sf::VideoMode(770, 512), "Chess", sf::Style::Titlebar | sf::Style::Close);
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    if ((0 <= event.mouseButton.x) && (event.mouseButton.x <= 512) && (0 <= event.mouseButton.y) && (event.mouseButton.y <= 512)) {
                        ...
                        if (!chess.getSelected()) chess.selectPiece(buttonPos);
                        else chess.moveSelected(buttonPos);
